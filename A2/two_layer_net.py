...
def nn_forward_backward(
    ...
    loss = torch.sum(-torch.log(softmax[torch.arange(N), y]))
    loss /= N
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    y_pred =  (nn_forward_pass(params, X)[0]).max(dim=1)[1]
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-5, 1e-4, 1e-3, 1e-1]
    learning_rates = [1e-3, 1e-2, 1e-1, 1, 1e2]
    learning_rate_decays = [0.5, 0.9]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for h in hidden_sizes:
            for reg in regularization_strengths:
                for lr_dec in learning_rate_decays:
                    print(f'training with lr={lr}, h={h}, reg={reg}, lr_decay={lr_dec}')
                    
                    net = TwoLayerNet(input_size=3072,
                                      hidden_size=h,
                                      output_size=10,
                                      device=X_train.device,
                                      dtype=X_train.dtype)
                    
                    stat_dict = net.train(X=X_train,
                                      y=y_train,
                                      X_val=X_val,
                                      y_val=y_val,
                                      num_iters=3000,
                                      batch_size=1000,
                                      learning_rate=lr,
                                      learning_rate_decay=lr_dec,
                                      reg=reg,
                                      verbose=False)
                    
                    if max(stat_dict['val_acc_history']) > best_val_acc:
                        best_val_acc = max(stat_dict['val_acc_history'])
                        best_net = net
                        best_stat = stat_dict
    #############################################################################
    #                               END OF YOUR CODE                            #
