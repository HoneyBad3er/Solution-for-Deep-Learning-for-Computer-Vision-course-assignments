...
def multiples_of_ten(start: int, stop: int):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_10th = 10*(start//10 + 1)
    if start_10th < stop:
        x = torch.arange(start=start_10th, end=stop, step=10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M = x.shape[0]
    y = torch.zeros((M, 4), dtype=x.dtype)
    
    all_rows_idx = list(range(M))
    x_first_column = x[all_rows_idx, [0]*M]
    
    y[all_rows_idx, [0]*M] = x_first_column
    y[all_rows_idx, [1]*M] = x_first_column
    y[all_rows_idx, [2]*M] = x[all_rows_idx, [2]*M]
    y[all_rows_idx, [3]*M] = x[all_rows_idx, [1]*M]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M, N = x.shape
    y = torch.zeros((M, N), dtype=x.dtype)

    all_columns_idx = list(range(N))
    all_rows_idx = list(range(M))
    all_rows_reversed_idx = all_rows_idx[::-1]
    
    for column_idx in all_columns_idx:
        y[all_rows_idx, [column_idx]*M] = x[all_rows_reversed_idx, [column_idx]*M]
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    y[torch.arange(x.shape[0]), x.min(dim=1)[1]] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M, N = x.shape
    avg_per_column_v = x.sum(dim=0) / M
    std_per_column_v = ((x - avg_per_column_v).square().sum(dim=0) / (M - 1)).sqrt()
    y = (x - avg_per_column_v) / std_per_column_v
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
