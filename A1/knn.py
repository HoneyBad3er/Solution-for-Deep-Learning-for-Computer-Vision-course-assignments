...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = (x_train_flatten[i] - x_test_flatten[j]).square().sum().sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i, :] = (x_test_flatten - x_train_flatten[i]).square().sum(dim=1).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = (x@x.t()).diag().view(1, -1).t() + (y@y.t()).diag().view(1, -1) - 2*x@y.t()
    dists = dists.sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    knn_idx = dists.argsort(dim=0)[:k, :]
    knn_labels = y_train[knn_idx]
    y_pred = torch.tensor([Counter(knn_labels[:, j].tolist()).most_common(1)[0][0] for j in range(num_test)], dtype=torch.int64)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x = torch.vstack(tensors=[x_train_folds[j] for j in range(num_folds) if i != j])
            y = torch.hstack(tensors=[y_train_folds[j] for j in range(num_folds) if i != j])
            
            classifier = KnnClassifier(x_train=x, y_train=y)
            
            k_to_accuracies[k].append(classifier.check_accuracy(x_test=x_train_folds[i],
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    means = np.array([statistics.mean(acc) for acc in k_to_accuracies.values()]) 
    best_k = list(k_to_accuracies.keys())[means.argmax()]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
