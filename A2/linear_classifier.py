...
def svm_loss_vectorized(
    ...
    # margins: NxC, scores[range(N), y].view((-1, 1)) - correct class scores
    # reshaped for broadcasting, 
    margins = scores - scores[range(N), y].view((-1, 1)) + 1  # note delta = 1
    margins[range(N), y] = 0
    
    # loss - avg() on row losses, so we can sum up all margins matrix values,
    # because if loss != 0 then loss = margin
    # margins > 0 - NxC bool matrix, contains True if margin > 0 and j != yi
    loss = margins[margins > 0].sum() / N
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    margins[margins > 0] = 1
    margins[margins < 0] = 0
    margins[range(N), y] = -1 * margins.sum(dim=1)
    dW = (margins.t() @ X).t() / N
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-4, 1e-3, 2.5e-3, 5e-3, 7.5e-3]
    regularization_strengths = [1e-5, 5e-5, 1e-4, 5e-4, 1e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    test_pred = cls.predict(X_val)
    
    train_acc = 100 * (y_train == train_pred).mean(dtype=torch.float32).item()
    val_acc = 100 * (y_val == test_pred).mean(dtype=torch.float32).item()
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        scores -= scores.max()
        softmax = scores.exp() / scores.exp().sum()
...
def softmax_loss_vectorized(
    ...
    loss = torch.sum(-torch.log(softmax[torch.arange(num_train), y]))
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 2.5e-3, 5e-3, 7.5e-3, 1e-2]
    regularization_strengths = [5e-6, 1e-5, 5e-5, 1e-4, 5e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
