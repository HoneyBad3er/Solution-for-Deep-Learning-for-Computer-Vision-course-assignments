...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        Linear_w_ReLU_out, Linear_w_ReLU_cache = Linear_ReLU.forward(x=X,
                                                                     w=self.params['W1'],
                                                                     b=self.params['b1'])
        
        Linear_out, Linear_cache =  Linear.forward(x=Linear_w_ReLU_out,
                                                   w=self.params['W2'],
                                                   b=self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ...
        # loss: softmax -> linear -> relu -> linear
        loss, dx = softmax_loss(Linear_out, y)
        loss += self.reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
        
        dx, dw, db  = Linear.backward(dout=dx,
                                      cache=Linear_cache)
        
        grads['W2'] = dw + 2 * self.reg * W2
        grads['b2'] = db

        dx, dw, db  = Linear_ReLU.backward(dout=dx,
                                           cache=Linear_w_ReLU_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Replace "pass" statement with your code
        h_dim = input_dim
        for h_num, hidden_dim in enumerate(hidden_dims):
          self.params[f'W{h_num + 1}'] = weight_scale * torch.randn(h_dim, hidden_dim, dtype=dtype, device=device)
          self.params[f'b{h_num + 1}'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
          h_dim = hidden_dim
        
        self.params[f'W{h_num + 2}'] = weight_scale * torch.randn(h_dim, num_classes, dtype=dtype, device=device)
        self.params[f'b{h_num + 2}'] = torch.zeros(num_classes, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for layer_num in range(self.num_layers - 1):
          w = self.params[f'W{layer_num + 1}']
          b = self.params[f'b{layer_num + 1}']
          x, caches[f'Linear_ReLU{layer_num + 1}'] = Linear_ReLU.forward(x=x, w=w, b=b)
          
          if self.use_dropout:
            x, caches[f'Dropout{layer_num + 1}'] = Dropout.forward(x=x, dropout_param=self.dropout_param)

        w = self.params[f'W{layer_num + 2}']
        b = self.params[f'b{layer_num + 2}']
        x, caches[f'Linear{layer_num + 2}'] = Linear.forward(x=x, w=w, b=b)
        ...
        dx, dw, db  = Linear.backward(dx, caches[f'Linear{self.num_layers}'])
        grads[f'W{self.num_layers}'] = dw + 2 * self.reg * w
        grads[f'b{self.num_layers}'] = db
        
        for layer_num in range(self.num_layers - 1)[::-1]:
          w = self.params[f'W{layer_num + 1}']

          if self.use_dropout:
            dx = Dropout.backward(dx, caches[f'Dropout{layer_num + 1}'])

          dx, dw, db  = Linear_ReLU.backward(dx, caches[f'Linear_ReLU{layer_num + 1}'])
          grads[f'W{layer_num + 1}'] = dw + 2 * self.reg * w
          grads[f'b{layer_num + 1}'] = db
...
def adam(w, dw, config=None):
    ...
    w -= (config['learning_rate'] * mt)/ (torch.sqrt(vc) + config['epsilon'])
    next_w = w
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            dropout_mask = torch.rand(x.shape) > p
            out = x.clone()
        ...
        cache = (dropout_param, dropout_mask)
